...
function compatibility (u1, u2) {
  //u1、u2 are user's name
  var profile = users[u2].u; //the u2's attribute
  var attributeScores = [];
  for (var attribute in users[u1].p) { //the attributes of user's who talk with u1
    var v = users[u1].p[attribute]; //attribute is 'gender'or'age'or'body' to v
    var n = 0;
    var d = 0;
    for (var jj in users[u1].p[attribute]) { //jj is '20-24':5/'25-29':20
      d += users[u1].p[attribute][jj]; // d+=5

      switch (attribute) {
        case 'gender':
        case 'body':
          if (jj == profile[attribute]) { //if the attribute of user's who talked with u1 are similar to u2
...
console.log('Table:');
var keys = Object.keys(users); // get the variable'users' 's keys(user's name)
for (var ii in keys) { 
  var u1 = keys[ii];
  for (var jj in keys) {
    var u2 = keys[jj];
